...
type StitchClient interface {
	...
	CreateEmptyApp(groupID, appName string, location string, deploymentModel string) (*models.App, error)
...
func (sc *basicStitchClient) CreateEmptyApp(groupID, appName string, location string, deploymentModel string) (*models.App, error) {
	res, err := sc.ExecuteRequest(
		http.MethodPost,
